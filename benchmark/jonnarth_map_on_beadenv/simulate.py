"""
benchmark/jonnarth_map_on_beadenv/simulate.py

Animated replay of our trained PPO model tracing the contour of Jonnarth et
al.'s eval_mowing_9.png map image, inside our own unmodified BeadEnv. Same
purpose/scope caveats as evaluate.py in this folder -- read its module
docstring first. This is a straight adaptation of the project's own
simulate.py to this benchmark folder and this one target image; no
env/model/reward logic is duplicated or changed.

Usage:
    python benchmark/jonnarth_map_on_beadenv/simulate.py --device cuda
    python benchmark/jonnarth_map_on_beadenv/simulate.py --save out.gif --no-show
"""
...
def build_animation(env: BeadEnv, frames: list, traj: list, image_path: str,
                     model_path: str, fps: int, speed: float):
    S = env.IMG_SIZE
    ...
    ax.set_title(f"BeadEnv PPO on Jonnarth et al.'s map -- {os.path.basename(image_path)}",
    ...
    def info_lines(f: dict) -> str:
        ...
        return (
            f"map             : {os.path.basename(image_path)}\n"
            f"step            : {f['steps']}\n"
...
def main():
    ...
    fig, ani = build_animation(env, frames, traj, args.image, args.model, fps=args.fps, speed=args.speed)
