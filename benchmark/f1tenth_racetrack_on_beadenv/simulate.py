"""
benchmark/f1tenth_racetrack_on_beadenv/simulate.py

Animated replay of our trained PPO model tracing an F1TENTH racetrack
outline (Elgouhary & El-Wakeel, arXiv:2602.18386's own dataset), inside our
own unmodified BeadEnv. Same purpose/scope caveats as evaluate.py in this
folder -- read its module docstring first.

Usage:
    python benchmark/f1tenth_racetrack_on_beadenv/simulate.py --track hockenheim --device cuda
    python benchmark/f1tenth_racetrack_on_beadenv/simulate.py --track montreal --save out.gif --no-show
"""
...
def build_animation(env: BeadEnv, frames: list, traj: list, track: str, fps: int, speed: float):
    S = env.IMG_SIZE
    ...
    ax.set_title(f"BeadEnv PPO on F1TENTH track -- {track}",
...
def main():
    ...
    print(f"Simulating {args.model} on F1TENTH track {args.track} (device={args.device})")
    ...
    fig, ani = build_animation(env, frames, traj, args.track, fps=args.fps, speed=args.speed)
